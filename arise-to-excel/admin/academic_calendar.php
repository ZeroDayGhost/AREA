    ...
    if ($action === 'set_active') {
        ...
        if (!$errors) {
            try {
                ...
                redirect('admin/academic_calendar.php?year=' . urlencode($activeYear));
        ...
        $calendar = $calendarId > 0 ? academic_calendar_by_id($pdo, $calendarId) : null;

        if (!$calendar) {
            $errors[] = 'Choose a calendar record to delete.';
        } else {
            $pdo->beginTransaction();
            try {
                $deletedYear = (string) $calendar['academic_year'];
                $deletedTerm = (string) $calendar['term_name'];
                $deletedActiveTerm = $deletedYear === $currentContext['academic_year'] && $deletedTerm === $currentContext['term'];
                $counts = purge_academic_term($pdo, $deletedYear, $deletedTerm, $calendarId);

                if ($deletedActiveTerm) {
                    $replacement = academic_calendar_replacement_context($pdo, $deletedYear, $deletedTerm);
                    if ($replacement) {
                        save_academic_setting($pdo, 'active_academic_year', $replacement['academic_year']);
                        save_academic_setting($pdo, 'active_term', $replacement['term']);
                    } else {
                        $clearSettings = $pdo->prepare(
                            "DELETE FROM academic_settings
                             WHERE setting_key IN ('active_academic_year', 'active_term')"
                        );
                        $clearSettings->execute();
                    }
                }

                $pdo->commit();
                if ($deletedActiveTerm && isset($replacement)) {
                    sync_current_term_fee_balances($pdo);
                }

                $removedParts = [];
                foreach ($counts as $label => $count) {
                    $removedParts[] = "{$count} {$label}";
                }
                flash('success', "{$deletedYear} {$deletedTerm} deleted completely. Removed " . implode(', ', $removedParts) . '.');
                redirect('admin/academic_calendar.php?year=' . urlencode($deletedYear));
            } catch (Throwable $exception) {
                $pdo->rollBack();
                $errors[] = 'Unable to delete term completely: ' . $exception->getMessage();
            }
        }
    } else {
        $calendarId = (int) ($_POST['calendar_id'] ?? 0);
                            ...
                            <tr class="<?= $isActiveRow ? 'table-primary' : '' ?>">
                                ...
                                <td>
                                    <div class="action-row">
                                        ...
                                        <form method="post" onsubmit="return confirm('Delete this term completely? This removes the calendar row plus all matching fee, balance, feeding, and transport records for this academic year and term.');">
                                            <input type="hidden" name="calendar_id" value="<?= (int) $row['id'] ?>">
                                            <button class="btn btn-sm btn-outline-danger" name="action" value="delete" type="submit">Delete Completely</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
