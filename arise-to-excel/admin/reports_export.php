...
function report_export_extra_sections(PDO $pdo, array $filters, array $feeTermRows, array $paymentHistory): array
{
    ...
    $kitchenExpenses = report_export_scalar($pdo, "SELECT COALESCE(SUM(total_amount), 0) FROM kitchen_inventory{$kitchenWhere}", $kitchenParams);
    $schoolExpenses = report_export_scalar($pdo, "SELECT COALESCE(SUM(total_amount), 0) FROM school_expenses{$expenseWhere}", $expenseParams);
    ...
    $expenseRows = [['Kitchen Inventory', report_export_money($kitchenExpenses)]];
